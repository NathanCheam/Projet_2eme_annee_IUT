...
/**
 * The CppCompiler class is used to compile C++ source files.
 ...
 */
public class CppProcess implements IProcess {
    ...
    public File compile(Judge judge, File sourceFile) {
        // Check if the file extension is .cpp, .cc, or .cxx
        if (!(sourceFile.getName().endsWith(".cpp") || sourceFile.getName().endsWith(".cc") || sourceFile.getName().endsWith(".cxx"))) {
            throw new IllegalArgumentException("The source file must be a .cpp, .cc, or .cxx file");
        }
        ...
        // Create the ProcessBuilder to compile with g++
        ProcessBuilder builder = new ProcessBuilder("g++", "-x", "c++", "-Wall", "-O2", "-static", "-pipe", "-o", compiledFile.getAbsolutePath(), sourceFile.getAbsolutePath());
